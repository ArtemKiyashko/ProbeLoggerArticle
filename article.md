...
В запуске, финальной сборке, поиске и спасении я, к сожалению, не смог поучавствовать. Так что в этих разделах буду повествовать со слов людей бывавших "на местах".
...
А вот сабж, в отличие от, для передачи координат использует **спутниковый канал связи** со 100% покрытием. Не буду наседать с рекламой, т.к. удовольствие это не дешевое, но отмечу - раз вы читаете эту статью, значит свою функцию он выполнил. Хотя и дал нам понервничать. Но об этом в разделе про поиск и спасение.

Так что еще раз - отнеситесь к выбору трекера максимально ответственно!
...
Это та часть, где время стерло бОльшую часть информации. Но поверьте - тут все просто и прозрачно, а на MSDN есть даже статься по отладке кода на RPI для VSCode и Visual Studio - [VS Remote Debug](https://docs.microsoft.com/en-us/dotnet/iot/debugging).
...
## Запуск
Выбрали точку, заправили тарантас, взяли пару баллонов гелия и двинули на место. Описывать тут особо нечего. Имейте только ввиду - шар такого размера с гелием имеет неплохую подъемную силу! Так что придерживайте его пока будете надувать (а лучше привяжите временно).
